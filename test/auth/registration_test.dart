...
void main() {
  group('AuthFeatureController - Registration Tests', () {
    ...
    test('signUp success', () async {
      ...
      final authUser = AuthUser(id: 'newlyCreatedUserId', email: email);
      when(
    ...
    test('signUp failure (email already in use)', () async {
      // Arrange
      final states = <AsyncValue<void>>[];
      authController.addListener(states.add, fireImmediately: false);

      final exception = Exception('Ten adres email jest już używany.');
      when(
        mockAuthRepository.signUp(email: email, password: password),
      ).thenThrow(exception);
